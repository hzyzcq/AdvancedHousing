...
#Since there are so many kinds of neighborhood, fitting them in a OneHot way may unwantedly
# thin out the importance of other attributes, so here it is replaced by an arbitray correlation
#bewtween neighborhood and sold price based on mean prices grouped by neighborhoods
...
def trans(housing):     
	...
	#Change year something happened to years from that event to the year the house was sold
	...
	global neigh
	#Encode neighborhood based on mean price 
	...
	#Transform some attribs based on explicit indication of fondness on Kaggle data description page
	...
	#Drop attributes that seems insignificant
...
#Based on statistics, a skewed dataset could inhibit the capacity of regressors that
#should take normally distributed ones as input
from scipy.stats import skew
#use log1p to redress skewness
...
#Check whether there are values that appear in train set but not test set
#Then drop these values in case they incur overfitting
...
param_distribs = {
	...
	'n_estimators':randint(low = 4000, high = 10000),
...
#One value that appears in test site but not train set
#Drop it to avoid error 
